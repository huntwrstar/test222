...
async function calculateComprehensive() {
    ...
    if (selectedEvents.length === 0) {
        ...
        if (tbody) tbody.innerHTML = `<tr><td colspan="5">${__('请至少选择一个项目')}</td></tr>`;
        return;
    }
...
function renderComprehensiveTable(data) {
    ...
    let theadHtml = `<tr><th>${__('table.rank')}</th><th>${__('table.name')}</th>`;
    if (source === 'province') {
        theadHtml += `<th>${__('table.province')}</th><th>${__('table.city')}</th>`;
    } else {
        theadHtml += `<th>${__('table.country')}</th>`;
    }
    theadHtml += `<th>${__('table.events_count')}</th><th>${__('table.total_rank')}</th></tr>`;
    thead.innerHTML = theadHtml;

    if (data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="${source === 'province' ? 6 : 5}">${__('no_data')}</td></tr>`;
        return;
    }
...
function updateRecordCitySelect(province) {
    ...
    if (MUNICIPALITIES.includes(province)) {
        ...
        let options = `<option value="全部城市">${__('all_cities')}</option>`;
...
async function loadRecordData() {
    ...
    if (!tbody) return;
    if (!state.record.dataLoaded) {
        tbody.innerHTML = `<tr><td colspan="5" class="loading-cell"><i class="fas fa-spinner"></i> ${__('数据未就绪')}</td></tr>`;
        return;
    }
    ...
    let displayCity = (city === '全部城市') ? __('all_cities') : city;
    document.getElementById('record-current-city').textContent = displayCity;
    let genderText = __('gender.all');
    if (gender === '男') genderText = __('gender.male');
    else if (gender === '女') genderText = __('gender.female');
    else if (gender === '未知') genderText = __('gender.unknown');
    document.getElementById('record-current-gender').textContent = genderText;

    tbody.innerHTML = `<tr><td colspan="5" class="loading-cell"><i class="fas fa-spinner"></i> ${__('loading')}<span class="loading-dots"></span></td></tr>`;
    ...
    for (let proj of PROJECT_LIST) {
        ...
        if (!singleRec && !avgRec) {
            html += `<tr><td colspan="5" class="empty-cell">${__('record.no_record')}</td></tr>`;
        } else {
            if (singleRec) {
                const displayName = getDisplayName(singleRec);
                html += `<tr>
                    <td></td>
            ...
            if (avgRec) {
                const displayName = getDisplayName(avgRec);
                html += `<tr>
                    <td></td>
                    <td></td>
    ...
    tbody.innerHTML = html || `<tr><td colspan="5">${__('no_data')}</td></tr>`;
}
...
async function updateCompCitySelect(province) {
    ...
    try {
        ...
        citySelect.disabled = false;
        citySelect.innerHTML = `<option value="全部城市">${__('all_cities')}</option>` + 
...
async function initLanguage() {
    ...
    if (savedLang && LANG_LIST.some(l => l.code === savedLang)) {
